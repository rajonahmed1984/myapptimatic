# React Pilot Staging Checklist: Expenses Index

## Scope
- Route: `GET /admin/expenses` (`admin.expenses.index`)
- Flag: `FEATURE_ADMIN_EXPENSES_INDEX`
- Fallback: Blade view remains default when flag is `false`

## Enable
1. Set staging env:
   - `FEATURE_ADMIN_EXPENSES_INDEX=true`
2. Rebuild caches:
...
2. UI parity:
   ...
   - ON returns Inertia page (`Admin/Expenses/Index`).
3. Behavior parity:
   ...
   - Navigation links to recurring, categories, and add expense remain unchanged.

## Safety Gates
- `php artisan test --filter=ExpensesUiParityTest`
- `php artisan test`
- `npm run build`
- `php artisan config:cache`
- `php artisan route:cache`
...
## Rollback
1. Set `FEATURE_ADMIN_EXPENSES_INDEX=false`
2. Run:
   - `php artisan optimize:clear`
   - `php artisan config:cache`
   - `php artisan route:cache`
