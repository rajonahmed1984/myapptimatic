...
const statusClass = (label) => {
    ...
    if (key.includes('paid')) return 'border-emerald-200 bg-emerald-50 text-emerald-700';
    if (key.includes('partial')) return 'border-blue-200 bg-blue-50 text-blue-700';
    if (key.includes('unpaid') || key.includes('pending')) return 'border-amber-200 bg-amber-50 text-amber-700';
            ...
            <DataTable
                ...
                columns={[
                    ...
                    },
                    {
                        ...
                        header: 'Status',
                        render: (item) => (
                            <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold ${statusClass(item.status_label)}`}>
                                {item.status_label}
                            </span>
                        ),
                    },
