...
const statusClass = (label) => {
    ...
    if (key.includes('approved') || key.includes('accepted')) return 'border-emerald-200 bg-emerald-50 text-emerald-700';
    if (key.includes('rejected') || key.includes('declined')) return 'border-rose-200 bg-rose-50 text-rose-700';
        ...
        <>
            ...
            <div className="space-y-6">
                <div className="rounded-2xl border border-slate-200/90 bg-white p-5 sm:p-6 shadow-sm">
                    ...
                    <div className="text-xl sm:text-2xl font-semibold text-slate-900">Request leave</div>
                    <div className="text-xs sm:text-sm text-slate-500">Submit a new request and track approvals.</div>
                ...
                <DataTable
                    ...
                    columns={[
                        ...
                        { key: 'days', header: 'Days', render: (leave) => leave.total_days },
                        {
                            ...
                            header: 'Status',
                            render: (leave) => (
                                <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold ${statusClass(leave.status_label)}`}>
                                    {leave.status_label}
                                </span>
                            ),
                        },
