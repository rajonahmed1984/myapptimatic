            ...
            <div className="card p-6 max-w-2xl">
                <form method="POST" action={routes?.update} data-native="true" className="grid gap-4 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input id="periodKey" name="period_key" defaultValue={period?.period_key || ''} className="ui-input mt-1" placeholder="YYYY-MM" required />
                    </div>
                    <div>
                        <label htmlFor="startDate" className="text-xs uppercase tracking-[0.2em] text-slate-500">Start Date</label>
                        <input id="startDate" type="text" placeholder="DD-MM-YYYY" inputMode="numeric" name="start_date" defaultValue={period?.start_date || ''} className="ui-input mt-1" required />
                    </div>
                    <div>
                        <label htmlFor="endDate" className="text-xs uppercase tracking-[0.2em] text-slate-500">End Date</label>
                        <input id="endDate" type="text" placeholder="DD-MM-YYYY" inputMode="numeric" name="end_date" defaultValue={period?.end_date || ''} className="ui-input mt-1" required />
                    </div>
