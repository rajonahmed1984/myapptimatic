        ...
        <>
            ...
            <div className="card p-6">
                <form method="POST" action={routes.store} data-native="true" className="mt-2 grid gap-4 rounded-2xl border border-slate-300 bg-white/80 p-5" encType="multipart/form-data">
                    ...
                    <div className="grid gap-4 md:grid-cols-3">
                        <div><label className="text-xs text-slate-500">Start date</label><input name="start_date" type="date" defaultValue={form.start_date || ''} className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm" /></div>
                        <div><label className="text-xs text-slate-500">Expected end date</label><input name="expected_end_date" type="date" defaultValue={form.expected_end_date || ''} className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm" /></div>
                        <div><label className="text-xs text-slate-500">Due date (internal)</label><input name="due_date" type="date" defaultValue={form.due_date || ''} className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm" /></div>
                    </div>
                        ...
                        <div className="space-y-3">
                            {maintenanceRows.map((maintenance, index) => (
                                <div key={maintenance.id} className="grid gap-3 md:grid-cols-6 mb-2">
                                    ...
                                    <div><label className="text-xs text-slate-500">Start date</label><input name={`maintenances[${index}][start_date]`} type="date" defaultValue={maintenance.start_date || ''} className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm" required /></div>
                            ...
                            {taskRows.map((task, index) => (
                                <div key={task.id} className="rounded-xl border border-slate-100 bg-white p-3">
                                    <div className="grid gap-3 md:grid-cols-5 mb-3">
                                        ...
                                        <div><label className="text-xs text-slate-500">Start date</label><input type="date" name={`tasks[${index}][start_date]`} defaultValue={task.start_date || ''} className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm" required /></div>
                                        <div><label className="text-xs text-slate-500">Due date</label><input type="date" name={`tasks[${index}][due_date]`} defaultValue={task.due_date || ''} className="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm" required /></div>
