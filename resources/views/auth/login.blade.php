        ...
        <div class="relative z-10">
            ...
            <form class="mt-8 space-y-5" method="POST" action="{{ route('login.attempt', [], false) }}">
                ...
                <div class="flex items-center justify-between text-sm text-slate-200/85">
                    ...
                    <a href="{{ route('password.request') }}" class="text-teal-300 hover:text-teal-200">Forgot password?</a>
                </div>
            ...
            <p class="mt-6 text-xs text-slate-200/85">
                Need an account?
                <a href="{{ route('register', request('redirect') ? ['redirect' => request('redirect')] : []) }}" class="font-semibold text-teal-300 hover:text-teal-200">Register</a>.
            </p>
        </div>
    </section>
