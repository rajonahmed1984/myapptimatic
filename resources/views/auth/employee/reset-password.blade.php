...
@section('content')
    <section class="relative -m-8 overflow-hidden rounded-2xl bg-slate-900 px-8 py-10 text-white sm:px-10">
        ...
        <div class="relative z-10">
            ...
            <form class="mt-8 space-y-5" method="POST" action="{{ route('employee.password.update') }}">
                @csrf
            ...
            <p class="mt-6 text-xs text-slate-200/85">
                Back to <a href="{{ $loginRoute ?? route('employee.login') }}" class="font-semibold text-teal-300 hover:text-teal-200">sign in</a>.
            </p>
        </div>
    </section>
@endsection
