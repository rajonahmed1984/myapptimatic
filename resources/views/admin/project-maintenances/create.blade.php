    ...
    <div class="card p-6 max-w-3xl">
        <form method="POST" action="{{ route('admin.project-maintenances.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <select name="project_id" id="project_id" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
            ...
            <div>
                ...
                <input name="title" value="{{ old('title') }}" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm" placeholder="Annual Hosting & Support">
            </div>
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    <label class="text-xs text-slate-500">Amount</label>
                    <input name="amount" type="number" min="0.01" step="0.01" value="{{ old('amount') }}" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
                </div>
                <div>
                    <label class="text-xs text-slate-500">Billing cycle</label>
                    <select name="billing_cycle" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input name="start_date" type="date" value="{{ old('start_date') }}" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
                </div>
            ...
            <div class="flex justify-end gap-3 pt-2">
                <a href="{{ route('admin.project-maintenances.index') }}" class="rounded-full border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 hover:border-teal-300 hover:text-teal-600">Cancel</a>
