...
@section('content')
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div class="flex-1">
            <form id="maintenanceSearchForm" method="GET" action="{{ route('admin.project-maintenances.index') }}" class="flex items-center gap-3">
                <div class="relative w-full max-w-sm">
                    <input
                        ...
                        hx-get="{{ route('admin.project-maintenances.index') }}"
                        hx-trigger="keyup changed delay:300ms"
                        hx-target="#maintenanceTable"
                        hx-swap="outerHTML"
                        hx-push-url="true"
                        hx-include="#maintenanceSearchForm"
                    />
                </div>
            </form>
        </div>
