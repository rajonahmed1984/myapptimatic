        ...
        <div class="card p-6 md:col-span-2">
            <form method="POST" action="{{ route('admin.project-maintenances.update', $maintenance) }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input name="title" value="{{ old('title', $maintenance->title) }}" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
                </div>
                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        <label class="text-xs text-slate-500">Amount</label>
                        <input name="amount" type="number" min="0.01" step="0.01" value="{{ old('amount', $maintenance->amount) }}" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
                    </div>
                    <div>
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <input name="start_date" type="date" value="{{ old('start_date', $maintenance->start_date?->format('Y-m-d')) }}" required class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
                    </div>
