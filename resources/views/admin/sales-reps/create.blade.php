        ...
        <form action="{{ route('admin.sales-reps.store') }}" method="POST" enctype="multipart/form-data" autocomplete="off" class="mt-6 space-y-6">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input name="email" type="email" value="{{ old('email') }}" class="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm" />
                </div>
                <div>
                    <label class="text-sm text-slate-600">Phone</label>
                    <input name="phone" value="{{ old('phone') }}" class="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm" />
                </div>
                <div>
                    <label class="text-sm text-slate-600">Password</label>
                    <input name="user_password" type="password" class="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm" />
                ...
                <div>
                    ...
                    <select name="employee_id" class="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm">
                ...
                <div class="md:col-span-2 grid gap-4 md:grid-cols-3">
                    <div>
                        ...
                        <input name="avatar" type="file" accept=".jpg,.jpeg,.png,.webp" class="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm" />
                        <p class="mt-1 text-xs text-slate-500">JPG, PNG, or WebP up to 2MB.</p>
                    </div>
                    <div>
                        <label class="text-sm text-slate-600">NID</label>
                        <input name="nid_file" type="file" accept=".jpg,.jpeg,.png,.webp" class="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm" />
                        <p class="mt-1 text-xs text-slate-500">JPG, PNG, or PDF up to 10MB.</p>
                    </div>
                    <div>
                        <label class="text-sm text-slate-600">CV</label>
                        <input name="cv_file" type="file" accept=".pdf" class="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm" />
