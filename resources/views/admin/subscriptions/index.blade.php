...
@section('content')
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div class="flex-1">
            <form id="subscriptionsSearchForm" method="GET" action="{{ route('admin.subscriptions.index') }}" class="flex items-center gap-3">
                <div class="relative w-full max-w-sm">
                    <input
                        ...
                        hx-get="{{ route('admin.subscriptions.index') }}"
                        hx-trigger="keyup changed delay:300ms"
                        hx-target="#subscriptionsTable"
                        hx-swap="outerHTML"
                        hx-push-url="true"
                        hx-include="#subscriptionsSearchForm"
                    />
                </div>
            </form>
        </div>
