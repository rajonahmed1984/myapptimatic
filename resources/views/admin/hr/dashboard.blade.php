...
@section('content')
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
            ...
            <div class="text-sm text-slate-500">Monitor employees, work logs, and payroll periods.</div>
        </div>
        <div class="flex flex-wrap items-center gap-2">
            ...
            <a href="{{ route('admin.hr.timesheets.index') }}" class="rounded-full border border-slate-200 px-4 py-2 text-xs font-semibold text-slate-700 hover:border-teal-300 hover:text-teal-600">Work logs</a>
        ...
        <div>
            ...
            <div class="mt-3 grid gap-4 md:grid-cols-3">
                ...
                <a href="{{ route('admin.hr.timesheets.index') }}" class="card px-4 py-3 leading-tight transition hover:border-teal-300 hover:shadow-sm">
                    <div class="flex items-center justify-between gap-3">
                        <div>
                            <div class="text-xs uppercase tracking-[0.25em] text-slate-400">Work log days (7d)</div>
                            <div class="text-xl font-semibold text-slate-900">{{ $pendingTimesheets }}</div>
                        </div>
                    </div>
                    <div class="mt-1 text-[11px] text-slate-500">Generated from Work Session data</div>
                </a>
                <a href="{{ route('admin.hr.timesheets.index') }}" class="card px-4 py-3 leading-tight transition hover:border-teal-300 hover:shadow-sm">
                    <div class="flex items-center justify-between gap-3">
                        <div>
                            <div class="text-xs uppercase tracking-[0.25em] text-slate-400">On-target days (7d)</div>
                            <div class="text-xl font-semibold text-slate-900">{{ $approvedTimesheets }}</div>
                        </div>
                    </div>
                    <div class="mt-1 text-[11px] text-slate-500">Active seconds reached required seconds</div>
                </a>
            </div>
        </div>
        ...
        <div class="card p-6">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    ...
                    <div class="text-sm text-slate-500">Latest work logs and leave requests.</div>
                </div>
                <div class="text-xs text-slate-400">Under-target days (7d): {{ $lockedTimesheets }}</div>
            </div>
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <div class="text-xs text-slate-500">
                                        {{ $sheet->work_date?->format($globalDateFormat ?? 'Y-m-d') ?? '--' }}
                                        -
                                        Active: {{ number_format(((int) ($sheet->active_seconds ?? 0)) / 3600, 2) }}h
                                    </div>
                                </div>
                                <div class="text-xs text-slate-500">
                                    {{ (int) ($sheet->required_seconds ?? 0) > 0 && (int) ($sheet->active_seconds ?? 0) >= (int) ($sheet->required_seconds ?? 0) ? 'On target' : 'Under target' }}
                                </div>
                            </div>
                        @empty
