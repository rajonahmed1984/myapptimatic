        ...
        <form method="POST" action="{{ route('admin.invoices.store') }}" class="space-y-6">
            ...
            <div class="grid gap-4 md:grid-cols-3">
                ...
                <div>
                    ...
                    <input name="issue_date" type="date" value="{{ old('issue_date', $issueDate ?? now()->toDateString()) }}" required class="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm" />
                    @error('issue_date')
                        <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    ...
                    <input name="due_date" type="date" value="{{ old('due_date', $dueDate ?? now()->toDateString()) }}" required class="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm" />
                    @error('due_date')
                        <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                    @enderror
                </div>
                <div class="md:col-span-3">
                    ...
                    <textarea name="notes" rows="3" class="mt-2 w-full rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm">{{ old('notes') }}</textarea>
                </div>
            </div>
            ...
            <div>
                <div class="flex items-center justify-between">
                    ...
                    <button type="button" id="addInvoiceItem" class="rounded-full border border-slate-300 px-3 py-1 text-xs font-semibold text-slate-600 hover:border-teal-300 hover:text-teal-600">Add item</button>
                </div>
                @error('items')
                    ...
                    @foreach($items as $index => $item)
                        <div class="invoice-item grid gap-3 md:grid-cols-12 items-start">
                            <div class="md:col-span-7">
                                ...
                                <input name="items[{{ $index }}][description]" value="{{ $item['description'] ?? '' }}" required class="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm" />
                            </div>
                            <div class="md:col-span-2">
                                <label class="text-xs text-slate-500">Qty</label>
                                <input name="items[{{ $index }}][quantity]" type="number" min="1" value="{{ $item['quantity'] ?? 1 }}" required class="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm" />
                            </div>
                            <div class="md:col-span-2">
                                <label class="text-xs text-slate-500">Unit Price</label>
                                <input name="items[{{ $index }}][unit_price]" type="number" min="0" step="0.01" value="{{ $item['unit_price'] ?? 0 }}" required class="mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm" />
                            </div>
                            <div class="md:col-span-1 flex items-end">
                                <button type="button" class="removeInvoiceItem rounded-full border border-rose-200 mt-7 px-3 py-2 text-xs font-semibold text-rose-600 hover:border-rose-300">Remove</button>
                            </div>
                        </div>
                    @endforeach
                </div>
