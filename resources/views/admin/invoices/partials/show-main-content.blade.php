    ...
    <div class="card p-6 space-y-6">
        ...
        <div id="manage-invoice-panel" class="hidden rounded-2xl border border-slate-200 bg-white/90 p-4">
            ...
            <form method="POST" action="{{ route('admin.invoices.update', $invoice) }}" class="grid gap-4 md:grid-cols-2" data-ajax-form="true">
                @csrf
                @method('PUT')
                <div>
        ...
        <div class="mt-6 flex flex-wrap justify-center gap-3">
            @if($invoice->status !== 'paid')
                <a
                    href="{{ route('admin.accounting.create', ['type' => 'payment', 'invoice_id' => $invoice->id, 'scope' => 'ledger']) }}"
                    data-ajax-modal="true"
                    data-url="{{ route('admin.accounting.create', ['type' => 'payment', 'invoice_id' => $invoice->id, 'scope' => 'ledger']) }}"
                    data-modal-title="Record Payment"
                    class="rounded-full bg-teal-500 px-5 py-2 text-sm font-semibold text-white"
                >Record payment</a>
                <form method="POST" action="{{ route('admin.invoices.recalculate', $invoice) }}" data-ajax-form="true">
                    @csrf
            ...
            @else
                <a
                    href="{{ route('admin.accounting.create', ['type' => 'refund', 'invoice_id' => $invoice->id, 'scope' => 'ledger']) }}"
                    data-ajax-modal="true"
                    data-url="{{ route('admin.accounting.create', ['type' => 'refund', 'invoice_id' => $invoice->id, 'scope' => 'ledger']) }}"
                    data-modal-title="Record Refund"
                    class="rounded-full border border-slate-200 px-5 py-2 text-sm font-semibold text-slate-600 hover:border-teal-300 hover:text-teal-600"
                >Record refund</a>
            @endif
        </div>
    </div>
