...
@section('content')
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div class="flex-1">
            <form id="customersSearchForm" method="GET" action="{{ route('admin.customers.index') }}" class="flex items-center gap-3">
                <div class="relative">
                    <input
                        type="text"
                        name="search"
                        ...
                        hx-get="{{ route('admin.customers.index') }}"
                        hx-trigger="keyup changed delay:300ms"
                        hx-target="#customersTable"
                        hx-swap="outerHTML"
                        hx-push-url="true"
                        hx-include="#customersSearchForm"
                    />
                </div>
            </form>
        </div>
