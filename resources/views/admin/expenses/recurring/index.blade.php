                ...
                <tbody>
                    @forelse($recurringExpenses as $recurring)
                        <tr class="border-b border-slate-100">
                            <td class="py-2 px-3 font-semibold text-slate-900">{{ $recurring->title }}</td>
                            ...
                            <td class="py-2 px-3 text-right">
                                <div class="flex justify-end gap-3 text-xs font-semibold">
                                    ...
                                    <form method="POST" action="{{ route('admin.expenses.recurring.generate') }}">
                                        @csrf
                                        <input type="hidden" name="template_id" value="{{ $recurring->id }}">
                                        <button type="submit" class="text-slate-500 hover:text-slate-700">Generate</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="py-4 px-3 text-center text-slate-500">No recurring expenses yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
