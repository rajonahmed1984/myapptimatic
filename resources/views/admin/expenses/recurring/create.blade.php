...
@section('page-title', 'Add Recurring Expense')

@section('content')
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
            <div class="section-label">Finance</div>
            <div class="text-2xl font-semibold text-slate-900">New recurring expense</div>
        </div>
        <a href="{{ route('admin.expenses.recurring.index') }}" class="rounded-full border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600 hover:border-teal-300 hover:text-teal-600">Back</a>
    </div>

    <div class="card p-6 max-w-3xl">
        <form method="POST" action="{{ route('admin.expenses.recurring.store') }}" class="grid gap-4 text-sm">
            @csrf
            <div>
                ...
                <div>
                    ...
                    <div class="mt-1 flex gap-2">
                        <select name="recurrence_type" class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
                            <option value="monthly" @selected(old('recurrence_type', 'monthly') === 'monthly')>Monthly</option>
                            <option value="yearly" @selected(old('recurrence_type') === 'yearly')>Yearly</option>
                        </select>
                        <input type="number" min="1" name="recurrence_interval" value="{{ old('recurrence_interval', 1) }}" class="w-24 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm" title="Interval">
                    </div>
            ...
            <div class="grid gap-3 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="date" name="end_date" value="{{ old('end_date') }}" class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
                    @error('end_date')
                        <div class="mt-1 text-xs text-rose-600">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="flex justify-end">
                <button type="submit" class="rounded-full bg-slate-900 px-4 py-2 text-xs font-semibold text-white hover:bg-slate-800">Save Recurring</button>
            </div>
        </form>
    </div>
@endsection
