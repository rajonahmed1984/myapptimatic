                ...
                <tbody>
                    @foreach($paymentProofs as $proof)
                        <tr class="border-b border-slate-100">
                            ...
                            <td class="px-4 py-3">
                                <x-status-badge :status="$proof->status" full />
                            </td>
                            <td class="px-4 py-3 text-slate-500">{{ $proof->created_at->format($globalDateFormat) }}</td>
                            <td class="px-4 py-3 text-right">
                                <div class="flex flex-wrap items-center justify-end gap-2 text-xs">
                                    @if($proof->attachment_url)
                                        <a href="{{ route('admin.payment-proofs.receipt', $proof) }}" target="_blank" rel="noopener" class="rounded-full border border-slate-300 px-3 py-1 font-semibold text-slate-600 hover:border-teal-300 hover:text-teal-600">View receipt</a>
