    ...
    <section class="relative -m-8 overflow-hidden rounded-2xl bg-slate-900 px-8 py-10 text-white sm:px-10">
        ...
        <div class="relative z-10">
            ...
            <form class="mt-8 space-y-5" method="POST" action="{{ route('support.login.attempt', [], false) }}">
                ...
                <div class="flex items-center justify-between text-sm text-slate-200/85">
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="remember" value="1" class="rounded border-white/30 text-teal-500 focus:ring-teal-200" />
                        Remember me
                    </label>
                    <a href="{{ route('support.password.request') }}" class="text-teal-300 hover:text-teal-200">Forgot password?</a>
                </div>

                @if(config('recaptcha.enabled') && config('recaptcha.site_key'))
                    <div class="flex justify-center">
                        <div class="g-recaptcha" data-sitekey="{{ config('recaptcha.site_key') }}" data-action="SUPPORT_LOGIN"></div>
                    </div>
                @endif
            ...
            <p class="mt-6 text-xs text-slate-200/85">
                Back to <a href="{{ route('login') }}" class="font-semibold text-teal-300 hover:text-teal-200">main login</a>.
            </p>
        </div>
    </section>
