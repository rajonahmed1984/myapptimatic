...
<div id="ticketMainWrap">
    <div class="mb-6 flex flex-wrap items-start justify-between gap-4">
        ...
        <div class="flex flex-col items-end gap-3 text-sm">
            ...
            <form method="POST" action="{{ route('support.support-tickets.status', $ticket) }}" data-ajax-form="true">
                @csrf
                @method('PATCH')
    ...
    <div class="card mt-6 p-6">
        ...
        <form method="POST" action="{{ route('support.support-tickets.update', $ticket) }}" class="mt-4 grid gap-4 md:grid-cols-2" data-ajax-form="true">
            @csrf
            @method('PATCH')
            ...
            <div class="flex {{ $reply->is_admin ? 'justify-end' : 'justify-start' }}">
                <div class="max-w-2xl rounded-2xl border border-slate-300 bg-white px-5 py-4 text-sm shadow-sm">
                    <div class="flex items-center justify-between text-xs text-slate-500">
                        ...
                        <span>{{ $reply->created_at->format($globalDateTimeFormat) }}</span>
                    </div>
    ...
    <div class="card mt-8 p-6">
        ...
        <form method="POST" action="{{ route('support.support-tickets.reply', $ticket) }}" class="mt-4 space-y-4" enctype="multipart/form-data" data-ajax-form="true">
            @csrf
