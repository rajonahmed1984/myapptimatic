...
@section('content')
    ...
    <div class="mb-6 flex flex-wrap items-start justify-between gap-4">
        ...
        <div class="flex flex-col items-end gap-3 text-sm">
            ...
            <form method="POST" action="{{ route('support.support-tickets.status', $ticket) }}">
                @csrf
                @method('PATCH')
    ...
    <div class="card mt-6 p-6">
        ...
        <form method="POST" action="{{ route('support.support-tickets.update', $ticket) }}" class="mt-4 grid gap-4 md:grid-cols-2">
            @csrf
            @method('PATCH')
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                button.addEventListener('click', async () => {
                    ...
                    try {
                        const response = await fetch("{{ route('support.support-tickets.ai', $ticket) }}", {
                            method: 'POST',
                            headers: {
