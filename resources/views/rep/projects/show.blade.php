...
@section('content')
    ...
    <div class="card p-6">
        ...
        <div class="mt-6 rounded-2xl border border-slate-200 bg-white/80 p-4 text-sm text-slate-700">
            ...
            <form method="POST" action="{{ route('rep.projects.tasks.store', $project) }}" class="grid gap-3 md:grid-cols-6">
                @csrf
                    ...
                    <table class="min-w-full text-left text-sm">
                        ...
                        @foreach($tasks as $task)
                            <tr class="border-t border-slate-100 align-top">
                                ...
                                <td class="px-3 py-2">
                                    <form method="POST" action="{{ route('rep.projects.tasks.update', [$project, $task]) }}" class="space-y-2">
                                        @csrf
                                        @method('PATCH')
                                    ...
                                    @can('delete', $task)
                                        <form id="delete-task-{{ $task->id }}" method="POST" action="{{ route('rep.projects.tasks.destroy', [$project, $task]) }}" onsubmit="return confirm('Delete this task?');" class="hidden">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    @endcan
